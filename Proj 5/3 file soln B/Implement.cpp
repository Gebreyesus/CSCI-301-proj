...
void DL_List::remove_Last()
{
  ...
  else if (first != NULL)//we remove an item/node only if there are existing nodes to begin with/only remove when the list !empty
	{
...
Node*  DL_List::get_last()
{
	return DL_List:: last;
}
Node*  DL_List::get_first()
{
